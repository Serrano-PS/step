...
int main()
{
	...
	printf("Digite a quantidade pessoa: ");
	scanf("%d%*c", &tam);

	p = (pessoa*)malloc(tam * sizeof(pessoa));
	percorre(p, tam, inicializa_pessoa);
	percorre_cmp(p, tam, &a, capta_pessoa);
	percorre(p, tam, imprime_pessoa);

	if (percorre_cmp(p, tam, &a, cmp_idade_pessoa))
		printf("Pessoa com idade %d encontrada!!", a);

	if (percorre_cmp(p, tam, &b, cmp_idade_pessoa))
		printf("Pessoa com idade %d encontrada!!", b);

	if (percorre_cmp(p, tam, &c, cmp_salario_pessoa))
		printf("Pessoa com idade %.2lf encontrada!!", c);

	if (percorre_cmp(p, tam, &d, cmp_salario_pessoa))
		printf("Pessoa com idade %.2lf encontrada!!", d);
